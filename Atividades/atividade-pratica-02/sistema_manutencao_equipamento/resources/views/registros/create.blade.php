...
<form action="{{ route('registros.store') }}" method="post">
    ...
    <div class="my-4">
        ...
        @enderror">
            ...
            <option value="{{ $usuario->id }}" @if(old('users_id', -1)==$usuario->id)
                selected @endif
                >{{ $usuario->name }}</option>
            @endforeach
        </select>
    ...
    <div class="my-4">
        ...
        @enderror">
            <option value="" selected disabled>Selecione:</option>
            <option value="1" @if(old('tipo', -1)==1) selected @endif>Preventiva</option>
            <option value="2" @if(old('tipo', -1)==2) selected @endif>Corretiva</option>
            <option value="3" @if(old('tipo', -1)==3) selected @endif>Urgente</option>
        </select>
    </div>
